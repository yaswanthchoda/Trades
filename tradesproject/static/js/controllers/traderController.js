angular.module('myApp').controller('traderController', traderController);
traderController.$inject = ['$scope','$location','$http','$window','$localStorage','$state','$rootScope','$timeout'];
function traderController($scope,$location,$http,$window,$localStorage, $state, $rootScope, $timeout) {
$rootScope.$on('$stateChangeSuccess', function() {
    document.body.scrollTop = 0;
    document.documentElement.scrollTop = 0;
});
  $scope.add_item = function(item_info){
        $scope.successmsg = '';
        $scope.user_msg = '';
        $scope.errmsg = '';
